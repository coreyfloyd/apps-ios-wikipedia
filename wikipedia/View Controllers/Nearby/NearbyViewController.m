...
@interface NearbyViewController ()<WMFPullToRefreshViewDelegate>
...
- (void)viewDidLoad
{
    ...
    self.pullToRefreshView = [WMFPullToRefreshView defaultIndeterminateProgressViewWithScrollView:self.collectionView delegate:self];

    [self.pullToRefreshView pullToRefreshContentView].refreshPromptString = MWLocalizedString(@"nearby-pull-to-refresh-prompt", nil);
    [self.pullToRefreshView pullToRefreshContentView].refreshRunningString = MWLocalizedString(@"nearby-pull-to-refresh-is-refreshing", nil);
