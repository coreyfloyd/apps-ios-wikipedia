...
#define IS_3point5IN    ([[UIScreen mainScreen] bounds].size.height == 480)
#define IS_4IN          ([[UIScreen mainScreen] bounds].size.height == 568)

//Super Slow iPad Detection
...
#warning remove redefinition of NSLog so that we can still log important data in production builds (used DEBUGLOG instead)
...
//Log specific objects and structs
#define ERRORLOG(object)                   (NSLog(@"" @"%s:" # object @"Error! %@", __PRETTY_FUNCTION__, [object description]));
#define DECIMALLOG(object)                       (NSLog(@"" # object @" %d", object));
#define FLOATLOG(object)                       (NSLog(@"" # object @" %f", object));
#define OBJECTLOG(object)                 (NSLog(@"" @"%s:" # object @" %@", __PRETTY_FUNCTION__, [object description]));
...
#define ASSERT_OR_LOG(...)                          [[NSAssertionHandler currentHandler] handleFailureInFunction :[NSString stringWithCString:__PRETTY_FUNCTION__ encoding:NSUTF8StringEncoding] file :[NSString stringWithCString:__FILE__ encoding:NSUTF8StringEncoding] lineNumber : __LINE__ description : __VA_ARGS__]
...
#define ERRORLOG(object)
#define DECIMALLOG(object)
#define FLOATLOG(object)
#define OBJECTLOG(object)
...
#define ASSERT_OR_LOG(...) NSLog(@"%s %@", __PRETTY_FUNCTION__, [NSString stringWithFormat:__VA_ARGS__])
